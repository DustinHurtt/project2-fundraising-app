...
router.post("/create-campaign", isLoggedIn, (req, res, next) => {
  Campaign.create({
    ...
    goal: req.body.goal,
    rawDeadline: new Date(Date.now() + req.body.duration * 6.048e8),
    timeLeft: "",
  })
...
router.get("/my-campaigns", isLoggedIn, async function (req, res, next) {
  await Campaign.find({ owner: req.session.user._id })
    .cursor()
    .eachAsync(function (myCampaign) {
      myCampaign.timeLeft = Interval.fromDateTimes(
        new Date(Date.now()),
        myCampaign.rawDeadline
      )
        .toDuration(["days", "hours", "minutes", "seconds"])
        .toObject();
      myCampaign.percent = Math.round(
        (myCampaign.currentTotal / myCampaign.goal) * 100
      );
      myCampaign.donations = myCampaign.pledges.length;
      (myCampaign.readableTotal = myCampaign.currentTotal.toLocaleString(
        "en-US",
        {
          style: "currency",
          currency: "USD",
        ...
        (myCampaign.deadline = myCampaign.rawDeadline.toLocaleString("en-US", {
          weekday: "long",
          year: "numeric",
          month: "long",
          day: "numeric",
